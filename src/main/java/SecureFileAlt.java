...
public class SecureFileAlt {
    private TripleDES des;
    private byte[] init_vector = new byte[8];
    private String operation;

    public SecureFileAlt(String key_file, String _operation){
        try {
            readKey(key_file);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static void main(String[] args) throws IOException {
        if(args.length < 4){
            System.out.printf("Not enough arguments. %d arguments given, 4 arguments needed.%n", args.length);
            return;
        }
